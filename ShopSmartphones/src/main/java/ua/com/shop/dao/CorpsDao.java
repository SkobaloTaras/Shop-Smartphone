...
public interface CorpsDao extends JpaRepository<Corps, Integer>{
	...
	@Query("SELECT c FROM Corps c WHERE c.options=?1 and c.caseMaterial=?2 and c.height=?3 and c.width=?4 and c.depth=?5 and c.weight=?6 and c.warranty=?7 and c.touchId=?8 and c.measuringSystem.id=?9 and c.measuringSystem2.id=?10 and c.measuringSystem3.id=?11 and c.measuringSystem4.id=?12 and c.measuringSystem5.id=?13 and c.countryProducing.id=?14 and c.producer.id=?15 ")                           
	Corps findUnique(String options, String caseMaterial, Double height,
			...
			int countryProducingId, int producerId);
}
